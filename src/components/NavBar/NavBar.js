import React from 'react'
import './navbar.css'

function NavBar() {
    

    return (
        <div className='nav'>
 <div className='group'>
 <div>
    <div class="sign-up-LtHXTi" data-id="0:50">
 <div class="navbar-GM8rhO" data-id="0:70">
 <div class="bg-6wNjKN" data-id="0:71">
 </div>
 <div class="icons---cart-6wNjKN" data-id="0:72">
 <div class="cart-fDFr3t" data-id="I0:72;0:4613">
 <a href="#"><img src="data:image/svg+xml;base64," class="shape-2jeRo8 decoded loaded" data-id="I0:72;0:4616" /></a>
 </div>
 </div>
 <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAAXNSR0IArs4c6QAAAARnQU1BAACxjwv8YQUAAAAQSURBVHgBAQUA+v8AAAAAAAAFAAFkeJU4AAAAAElFTkSuQmCC" class="logo-6wNjKN decoded loaded" data-id="0:73"/>
 <div class="group-618-6wNjKN" data-id="0:74">
 <a href="#" style={{ textDecoration: 'none' }} class="pricing-01ne91" data-id="0:75">À propos</a>
 <a href="#" class="pricing-87LWd3" style={{ textDecoration: 'none' }} data-id="0:76">Devenir partenaire</a>
 <a href="#"class="pricing-VURckE" style={{ textDecoration: 'none' }}data-id="0:77">Contact</a>
 <a href="#"class="home-01ne91"style={{ textDecoration: 'none' }} data-id="0:78">Accueil</a>
 <div class="group-617-01ne91" data-id="0:79">

 <a href="#" class="about-us-H2XtHD"style={{ textDecoration: 'none' }} data-id="0:80">Shop</a>
 <img src="data:image/svg+xml;base64," class="loaded" data-id="I3:77;3:228" />

 </div>
 
 </div>
 <a href="/Login" class="buttons---primary-6wNjKN" data-id="0:82">
 <div class="button---primary-XFsEyK" data-id="I0:82;0:4712">
 <div class="btn-j3hFuD" data-id="I0:82;0:4713">
 </div>
 <p class="primary-j3hFuD" data-id="I0:82;0:4714">Connexion</p></div>
 </a>
 </div>
</div>
</div>
</div>
 <img src="data:image/png;base64," class="logo-tJE4Af decoded loaded" data-id="0:83"/>


 <div class="overlay-window" id="overlay-main-js">
 </div>

        </div>
    )
}

export default NavBar
